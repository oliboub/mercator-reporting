...
**Version 2.0 — Document Consolidé**
...
├── src/
│   ├── __init__.py
│   ├── config.py                      ← Settings pydantic (Mercator URL, login, password)
│   ├── main.py                        ← FastAPI app + routers
│   │
│   ├── core/
│   │   ├── __init__.py
│   │   ├── mercator_client.py         ← Client HTTP (auth, cache, retry)
│   │   └── dependencies.py            ← Injection FastAPI (singleton client)
│   │
│   ├── api/routes/
│   │   ├── __init__.py
│   │   └── mercator.py                ← 5 routes GET /api/mercator/...
│   │
│   ├── models/__init__.py             ← (stub Phase 3)
│   ├── reporting/__init__.py          ← (stub Phase 3)
│   └── services/__init__.py           ← (stub Phase 3)
│
├── tests/
│   ├── __init__.py
│   ├── conftest.py                    ← Fixtures + mocks données réelles
│   ├── test_phase1_api.py             ← 35 tests routes HTTP (US2)
│   └── test_phase1_mercator_client.py ← 23 tests unitaires client
│
├── pyproject.toml
├── bootstrap.sh
├── Dockerfile
...
├── .env.example
├── README.md
├── docs/PRD_mercator_reporting_v2.md
└── docs/ANNEXE_A_mapping_mercator.md
    └── sources/
        ├── mercator_backup_dump_v4.py
        └── dump_standard.json

```
### 5.2 Arborescence du container docker
...
- [ ] Critères définis et mesurables
- [ ] Instructions de développement rédigées
- [ ] Instructions de tests rédigées
- [ ] Schéma d'architecture Mermaid présent
- [ ] Stratégie agile avec étapes définies
- [ ] Document complet en Markdown livré
...
**Critères d'acceptance :**
- [ ] Liste filtrable de tous les équipements par catégorie
- [ ] Détails des informations par équipement
...
### Phase 1 — Analyse & Conception (Semaine 1-2)
```
├── Audit des API Mercator CMDB disponibles
├── Mapping des données et modèle conceptuel
├── Validation des user stories avec stakeholders
├── Architecture technique détaillée (ce PRD)
└── Mise en place environnement de développement
```
**Livrable :** PRD validé + arborescence projet initialisée

---

### Phase 2 — Socle Technique (Semaine 3-4)
```
├── Mise en place du projet Python (uv + pyproject.toml)
├── Connexion aux API Mercator CMDB (mercator_client.py)
├── Authentification par token (user/password -> token)
├── Tests d'intégration de base (curl + pytest)
└── Dockerisation de l'application
```
**Livrable :** Backend opérationnel, appel API Mercator fonctionnel (US2)

**Instructions de test Phase 2 :**
```bash
# Curl — vérifier la connexion Mercator
curl -X POST http://localhost:8000/api/auth/token \
  -H "Content-Type: application/json" \
  -d '{"username": "user", "password": "pass"}'

# Pytest — valider les endpoints
...
|--------|---------|-------------|
| Annexe A | `PRD_ANNEXE_A_mapping_mercator.md` | Mapping complet des données Mercator CMDB : endpoints, champs, relations, champs BIA/CIAT, graphe de dépendances et recommandations de jointures pour le ReportEngine |
